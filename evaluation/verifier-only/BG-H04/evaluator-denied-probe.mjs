...
const root = path.resolve(process.cwd(), "candidates/BG-H04");
async function denied(operation) { try { await operation(); return false; } catch (error) { return error?.code === "ERR_ACCESS_DENIED"; } }
const results = await Promise.all([() => readdir(root), () => stat(root), () => readFile(path.join(root, "candidate-a", "AstronomyChecklist.ts"))].map(denied));
