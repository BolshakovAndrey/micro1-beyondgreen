...
const candidateRoot = path.resolve(process.cwd(), "candidates/BG-H02");
...
const results = await Promise.all([() => readdir(candidateRoot), () => stat(candidateRoot), () => readFile(path.join(candidateRoot, "candidate-a", "StargazingGuidePreview.ts"))].map(denied));
