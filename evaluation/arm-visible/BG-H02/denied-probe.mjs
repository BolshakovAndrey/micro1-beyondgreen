...
async function denied(operation) { try { await operation(); return { denied: false, signature: "allowed" }; } catch (error) { return { denied: error?.code === "ERR_ACCESS_DENIED", signature: `${error?.code ?? "unknown"}:${error?.permission ?? "unknown"}` }; } }
const operations = { enumerate: () => readdir(verifierRoot), read: () => readFile(path.join(verifierRoot, "BG-H02", "ground-truth.json")), stat: () => stat(verifierRoot), access: () => access(verifierRoot) };
...
const existing = await denied(() => stat(verifierRoot)); const absent = await denied(() => stat(absentRoot));
