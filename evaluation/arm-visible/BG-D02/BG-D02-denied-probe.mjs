/** Emits only aggregate proof that an arm-visible process cannot access verifier-only files. */
import { access, readdir, readFile, stat } from "node:fs/promises";
import path from "node:path";

const verifierRoot = path.resolve(process.cwd(), "evaluation/verifier-only");
const missingRoot = path.resolve(process.cwd(), "evaluation/verifier-only-absent-control");
...
const operations = {
  enumerate: () => readdir(verifierRoot), read: () => readFile(path.join(verifierRoot, "BG-D02", "ground-truth.json")),
  stat: () => stat(verifierRoot), access: () => access(verifierRoot),
};
...
const absent = await denied(() => stat(missingRoot));
