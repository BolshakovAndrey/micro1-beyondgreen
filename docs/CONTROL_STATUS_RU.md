...
> **Текущий статусный слой — 2026-08-31:** раздел «Финальное текущее состояние»
> заменяет устаревшие выводы исторического чек-листа. Остальное содержание сохраняется
> как хронология управления и не должно использоваться как текущая панель состояния.
...
**Остаток на момент обновления:** финальная подготовка submission

## Финальное текущее состояние

- Реализация и официальный evidence bundle завершены и зафиксированы.
- RUN-002 содержит 40 неизменяемых решений: 20 baseline без модели и 20 BeyondGreen с
  одним вызовом `gpt-5.6-sol` и без повторов.
- `POSTDECISION-004` содержит 80 captures и 40 записей оценщика; при восстановлении не
  выполнялись плечи и не вызывалась модель.
- Итог: baseline 10/20, BeyondGreen 19/20, преимущество +9/20. Строгая метрика причины
  3/10 не достигла замороженной цели 8/10; результат не скрыт.
- Офлайн-повтор подтверждает SHA-256 доказательств
  `972cf43f76d0e68534b481a15bfc9f6b9e2db3bf83b4cb5c80d654d07b378f07`.
- Репетиция чистой распаковки, состав которой выведен из `config/cleanroom.json`, прошла
  для 1054 файлов с временным манифестом самой репетиции; release payload содержит
  1053 файла. Проверка лицензий прошла 54/54.
- Финальный монтаж длится 4:54, использует `40+ ITERATIONS` и содержит раскрытие помощи
  AI-агентов. Основная ссылка YouTube проверена в чистом профиле без входа; публичное
  зеркало Vimeo зафиксировано. Финальный checkpoint Claude не обнаружил блокера;
  владелец разрешил создание ZIP после повторных сканирований, checksums и preflight.
- Commit, push, публикация и загрузка остаются запрещены; создание одного финального ZIP
  разрешено только после полностью зелёного выпускного гейта SES-040.

## Историческое состояние до официального запуска

## Актуальное состояние перед единственным раскрытием результатов

- Текущий исходный checkpoint: `cb51e7de1e9908088dbef4dbeb29946d1edbc16b`.
- Все десять пакетов `BG-D01`–`BG-D04` и `BG-H01`–`BG-H06` интегрированы.
- Проверенная координаторская трасса
  `TRC-BG-HELDOUT-FIXTURES-INTEGRATION-001` продвинута в `actual_traces` коммитом
  `e08f2f91440a484df9f9c9ae772461bd3d9d49d1`.
- Ограниченное исправление TypeScript `TS2532` находится в текущем `HEAD`; Chromium
  при его переносе и на этом этапе не запускался.
- По одобренной границе `SES-20260830-015` общий контроль замороженного набора
  прошёл: компиляция и видимые проверки `20/20`, независимый оценщик — `10`
  корректных `accept` и `10` обоснованных `reject`, все манифесты совпали, взаимный
  физический запрет доступа к скрытому оракулу и точные проверки количества,
  разбиения и классов прошли.
- Команды `baseline:verify`, `beyondgreen:verify`, `evaluation:run` и `replay`
  зарегистрированы только как точки проверки контракта до раскрытия: они проверяют
  точную версию `eval-v1.1.0`, но не запускают arms — сравниваемые варианты, — не
  считают итоговый результат, не раскрывают отложенные примеры и не создают записи
  запусков. `replay` воспроизводит только существующее незачётное D01-доказательство
  без сети.
- Ранняя временная ZIP-репетиция после чистой распаковки прошла: compile, `115/115`
  обычных тестов, повторный полный контроль замороженного набора, четыре безопасные
  команды, автономное воспроизведение и аудит лицензий `54/54`. ZIP и временный
  каталог удалены; финальный архив не создан.
- Первая ZIP-попытка честно остановилась на локальном машинном пути в историческом
  `SES-20260830-014`. После отдельного одобрения владельца ровно две неиндексируемые
  служебные записи управляющего контура были исключены только из временного архива;
  их исходные SHA-256 и причины вошли во временный манифест. Байты репозитория и
  `docs/CONTROL_PLANE_SHA256SUMS` не менялись.
- Официальный/оцениваемый запуск, единственное раскрытие результатов, живая модель,
  Chromium, browser/MCP, Claude, commit и push не выполнялись и остаются запрещены.

**Следующий необратимый барьер:** отдельное одобрение владельца точного чистого
`HEAD`, замороженных хешей манифестов и оценщика, команд, единственной попытки,
правил безопасной остановки и запрета настройки v1.1 после раскрытия. Текущий
ограниченный checkpoint такого разрешения не даёт.
...
- [x] Андрей уточнил, что право публично показывать распознаваемую запись реального
  продукта не заявляется. Фрагмент допустим только в необратимо заблюренном виде, где
  продукт невозможно распознать, а любые читаемые данные являются синтетическими. Если
  это условие не выполнено хотя бы в одном кадре, фрагмент удаляется.
