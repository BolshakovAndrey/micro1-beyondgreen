  ...
  useImperativeHandle(ref, () => ({
    dispatch(action) {
      ...
      if (action.type === "reject" && pending.current) {
        state.value = Object.freeze({ displayedTheme: pending.current.committedBefore, committedTheme: pending.current.committedBefore, status: "idle", error: "save_failed", pendingCount: 0 });
        pending.current = null;
      }
