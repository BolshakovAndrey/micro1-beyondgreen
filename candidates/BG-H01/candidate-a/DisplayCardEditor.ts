...
export const DisplayCardEditor = React.forwardRef<DisplayCardEditorHandle, Omit<DisplayCardEditorProps, "ref">>(function DisplayCardEditor(props, ref) {
  ...
  useLayoutEffect(() => {
    ...
    if (current.current.cardId !== next.cardId) {
      // Both fields transition together so no settled observation combines drafts
      // from different cards.
      current.current = next;
      title.value = next.initialTitle;
      theme.value = next.initialTheme;
    }
    appliedRevision.current = props.revision;
    actionLog.value = [...actionLog.value, props.transitionLabel];
  }, [props.cardId, props.initialTheme, props.initialTitle, props.revision, props.transitionLabel, actionLog, theme, title]);
