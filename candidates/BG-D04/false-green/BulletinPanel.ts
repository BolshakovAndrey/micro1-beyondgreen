/** Implements the seeded BG-D04 false-green subscription-cleanup candidate. */
...
/** Deliberately preserves one defect family: obsolete channel subscriptions remain live. */
export const BulletinPanel = forwardRef<BulletinPanelHandle>(function BulletinPanel(_, ref) {
  ...
  useEffect(() => {
    ...
    bulletinSource.subscribe(subscribedChannel, (value) => {
      bulletin.value = `${subscribedChannel}:${value}`;
    });
    // Intentionally no cleanup: this is the sole seeded false-green defect.
  }, [channel.value]);
